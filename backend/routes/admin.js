...
router.patch('/fulfillment/cards/:cardId/dispatch', requireSeniorAdmin, async (req, res) => {
  ...
  if (trackingId.length < 3 || trackingId.length > 100 || /[\r\n]/.test(trackingId)) {
    return res.status(400).json({ error: 'Enter a valid tracking ID between 3 and 100 characters.' });
  }
  ...
  if (!card.dispatchedAt) card.dispatchedAt = new Date();
...
// A courier tracking number is entered only when this card purchase ships.
// Repeating the action edits the number without changing the first dispatch
// timestamp, so admin corrections do not rewrite the shipment history.
router.patch('/requests/:id/tracking', requireAdmin, async (req, res) => {
  try {
    const trackingId = typeof req.body.trackingId === 'string' ? req.body.trackingId.trim() : '';
    if (trackingId.length < 3 || trackingId.length > 100 || /[\r\n]/.test(trackingId)) {
      return res.status(400).json({ error: 'Enter a valid tracking ID between 3 and 100 characters.' });
    ...
    if (request.paymentStatus !== 'paid') {
      return res.status(400).json({ error: 'Tracking can only be added to a paid card order.' });
    }
    if (request.status === 'rejected') {
      return res.status(400).json({ error: 'Tracking cannot be added to a rejected request.' });
    }

    request.trackingId = trackingId;
    if (!request.dispatchedAt) request.dispatchedAt = new Date();
    await request.save();
    res.json(request);
  } catch (err) {
    console.error('[admin/requests tracking PATCH]', err);
    res.status(500).json({ error: 'Failed to save tracking ID' });
  }
});

router.patch('/requests/:id/deliver', requireAdmin, async (req, res) => {
  try {
    const request = await CardRequest.findById(req.params.id);
    if (!request) return res.status(404).json({ error: 'Request not found' });
    if (!request.trackingId || !request.dispatchedAt) {
      return res.status(400).json({ error: 'Add the courier tracking ID before marking this order delivered.' });
    }
    ...
    console.error('[admin/requests deliver PATCH]', err);
    res.status(500).json({ error: 'Failed to mark the card order delivered' });
  }
});
...
// (ordered -> shipping -> delivery -> completed). Admin must enter the
// courier tracking ID when moving an order to shipping; fabricated tracking
// numbers are never generated by the server.
...
router.patch('/magic-poster-orders/:id', requireAdmin, async (req, res) => {
  try {
    ...
    const currentStage = MAGIC_POSTER_STATUS_ORDER.indexOf(order.status);
    const requestedStage = MAGIC_POSTER_STATUS_ORDER.indexOf(status);
    const isTrackingEdit = requestedStage === currentStage && trackingId !== undefined;

    // Forward-only status changes. Re-submitting the current stage is only
    // allowed when admin is explicitly editing the courier tracking ID.
    if (!isTrackingEdit && requestedStage <= currentStage) {
      return res.status(400).json({ error: `Order is already at or past "${order.status}"` });
    }

    const cleanTrackingId = typeof trackingId === 'string' ? trackingId.trim() : '';
    if (trackingId !== undefined && (cleanTrackingId.length < 3 || cleanTrackingId.length > 100 || /[\r\n]/.test(cleanTrackingId))) {
      return res.status(400).json({ error: 'Enter a valid tracking ID between 3 and 100 characters.' });
    }
    if (status === 'delivery' && !order.trackingId && !cleanTrackingId) {
      return res.status(400).json({ error: 'Tracking ID is required to move an order from shipping to delivery.' });
    }
    if (isTrackingEdit && !cleanTrackingId) {
      return res.status(400).json({ error: 'Tracking ID cannot be empty.' });
    }

    if (!isTrackingEdit) order.status = status;
    if (cleanTrackingId) order.trackingId = cleanTrackingId;
    await order.save();
    res.json(order);
  } catch (err) {
...
// For a plan flagged isSpecialEdition ("Limited Edition"-style: every
// buyer gets the exact same fixed video/model, not an individually
// customized one), a per-client video/model upload from ClientDetail.jsx
// should land on the shared PLAN VARIANT itself -- the same subdocument
// Plans.jsx's own variant uploader writes to -- instead of a per-client
// override. That way editing from EITHER admin surface updates the one
// real shared value, and every other client on that same variant sees it
// too (not just the client whose page the edit was made from).
// Resolves to `card.cardVariantId`'s own variant when set; falls back to
// the plan's lone variant if it has exactly one and this card has none
// assigned yet (same fallback utils/cardVariant.js's resolveCardVariant
// uses). Returns null when no single variant can be determined (e.g.
// several variants on the plan, none assigned to this card) -- callers
// fall back to the normal per-client override in that case rather than
// guessing which variant to write to.
function resolveWritableVariant(plan, card) {
  if (!plan) return null;
  if (card.cardVariantId) return plan.variants.id(card.cardVariantId) || null;
...
// POST /api/admin/clients/:clientId/magic-card/video -- Special Edition
// (see resolveWritableVariant above) writes to the shared plan VARIANT
// instead of this one client's own doc.
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!loaded) return;
      const doc = await findOrCreateMagicCard(req.params.clientId, loaded.cardNumber);
      const plan = await CardPlan.findOne({ key: loaded.card.cardType });
      const variant = plan?.isSpecialEdition ? resolveWritableVariant(plan, loaded.card) : null;
      const fileUrl = `${process.env.BACKEND_URL}/uploads/magic-cards/${req.file.filename}`;
      const cropX = Number(req.body.cropX) || 0;
      const cropY = Number(req.body.cropY) || 0;
      const cropWidth = Number(req.body.cropWidth) || 1;
      const cropHeight = Number(req.body.cropHeight) || 1;
      if (variant) {
        const previousUrl = variant.videoUrl;
        variant.videoUrl = fileUrl;
        variant.videoCropX = cropX;
        variant.videoCropY = cropY;
        variant.videoCropWidth = cropWidth;
        variant.videoCropHeight = cropHeight;
        await plan.save();
        if (previousUrl) fs.unlink(path.join(MAGIC_CARD_DIR, path.basename(previousUrl)), () => {});
      } else {
        const previousUrl = doc.videoUrl;
        doc.videoUrl = fileUrl;
        doc.videoCropX = cropX;
        doc.videoCropY = cropY;
        doc.videoCropWidth = cropWidth;
        doc.videoCropHeight = cropHeight;
        doc.updatedBy = req.admin?.email || 'unknown';
        await doc.save();
        if (previousUrl) fs.unlink(path.join(MAGIC_CARD_DIR, path.basename(previousUrl)), () => {});
      }
...
// comment in models/MagicBusinessCard.js. Special Edition (see
// resolveWritableVariant above) writes to the shared plan VARIANT instead
// of this one client's own doc.
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const variant = plan?.isSpecialEdition ? resolveWritableVariant(plan, loaded.card) : null;
      const fileUrl = `${process.env.BACKEND_URL}/uploads/magic-cards/${req.file.filename}`;
      const modelType = path.extname(req.file.originalname).toLowerCase() === '.fbx' ? 'fbx' : 'glb';
      if (variant) {
        const previousUrl = variant.modelUrl;
        variant.modelUrl = fileUrl;
        variant.modelType = modelType;
        await plan.save();
        if (previousUrl) fs.unlink(path.join(MAGIC_CARD_DIR, path.basename(previousUrl)), () => {});
      } else {
        const previousUrl = doc.modelUrl;
        doc.modelUrl = fileUrl;
...
router.delete('/clients/:clientId/magic-card/:field', requireAdmin, async (req, res) => {
  try {
    ...
    // Special Edition (see resolveWritableVariant above) -- video/model
    // removal clears the shared plan VARIANT instead of this client's own
    // (never-written-to, for this plan type) doc fields.
    if (field === 'video' || field === 'model') {
      const plan = await CardPlan.findOne({ key: loaded.card.cardType });
      const variant = plan?.isSpecialEdition ? resolveWritableVariant(plan, loaded.card) : null;
      if (variant) {
        if (field === 'video') {
          if (variant.videoUrl) fs.unlink(path.join(MAGIC_CARD_DIR, path.basename(variant.videoUrl)), () => {});
          ...
          if (variant.modelUrl) fs.unlink(path.join(MAGIC_CARD_DIR, path.basename(variant.modelUrl)), () => {});
          variant.modelUrl = undefined;
          variant.modelType = undefined;
        }
        await plan.save();
        return res.json(await serializeMagicCard(doc, loaded.card));
      }
    }
