...
// page component (not an edit to Home.jsx) specifically so switching
// back is a one-line import swap in App.jsx, not a design revert.
// Everything below the hero (features grid, steps) is untouched from
// Home.jsx -- the reference only covered the hero.
...
const STATS = [
  { target: 13.56, suffix: ' MHz', label: 'NFC frequency' },
  { target: 888, suffix: ' bytes', label: 'NDEF capacity' },
  { target: 0, suffix: '', label: 'battery required', display: 'Zero' },
];
...
export default function HomeD1() {
  ...
  return (
    <div>
      <div className="d1-hero" ref={heroRef}>
        ...
        <div className="d1-hero-col-text">
          <span className="hero-eyebrow">NFC &middot; NTAG216 &middot; Password-locked</span>
          <h1 className="d1-headline">
            TAP ONCE,
            <br />
            <span className="outline">SHARE</span>
            <br />
            <span className="grad">YOURSELF</span>
          </h1>
          <p className="d1-lede">
            One tap transmits your full profile over near-field induction — no app on their end,
            no battery, no signal drop. Update your details anytime; every card in the world
            reflects it instantly.
          </p>
          <div className="hero-cta-row" style={{ justifyContent: 'flex-start' }}>
            <Link to="/shop" className="btn-primary" style={{ background: 'var(--d1-gradient)', color: '#1a0e04' }}>
              Shop Cards
            </Link>
          ...
          <div className="d1-stats">
            {STATS.map((s) => (
              <div className="d1-stat-card" key={s.label}>
                <b>{s.display || <CountUp target={s.target} suffix={s.suffix} />}</b>
                <span>{s.label}</span>
              </div>
            ))}
          </div>
        ...
        <div className="d1-hero-col-visual">
          ...
          <div className="d1-tagline-label">One card, always current</div>
          <p className="d1-tagline-caption">
            The card only holds a link, not your details — update your number, add a portfolio,
            or swap your photo any time, and everyone who taps it sees the new version.
          </p>
        </div>
      </div>
      ...
      <div className="d1-features-panel">
        ...
        <div className="d1-feature-grid">
          {FEATURES.map((f, i) => (
            <div className="d1-feature-card" key={f.title}>
              <div className="d1-feature-card-top">
                <div className="d1-feature-icon">{f.icon}</div>
                <span className="d1-feature-num">{String(i + 1).padStart(2, '0')}</span>
              </div>
              <h3>{f.title}</h3>
              <p>{f.desc}</p>
            </div>
      ...
      <div className="d1-steps-section">
        ...
        <p className="d1-steps-subheading">From order to first tap, in three steps.</p>
        <div className="d1-steps-row">
          {STEPS.map((s, i) => (
            <div className="d1-step-item" key={s.title}>
              <div className={`d1-step-number${i === 1 ? ' accent' : ''}`}>{i + 1}</div>
              <h4>{s.title}</h4>
              <p>{s.desc}</p>
            </div>
          ))}
      ...
      <div style={{ textAlign: 'center', marginTop: 48 }}>
        <Link
          ...
          style={{
            ...
            padding: '14px 32px',
            borderRadius: 999,
            background: 'linear-gradient(120deg, #ffcf5c, #ff8a3d, #ff5e1a)',
            color: '#1a0e04',
            fontWeight: 700,
            textDecoration: 'none',
          }}
